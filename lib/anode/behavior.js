...
var behavior = exports.behavior = function () {
  ...
  if ( typeof beh === 'function' ) {
    ...
    matchers.push( 
      _createMatcher( [ { type: '_' } ], beh.toString() ) );
    ...
    for ( var pattern in beh ) {
      ...
      var functionBody = beh[ pattern ].toString();
    ...
    return function () {
      ...
      for ( var j = 0; j < matchers.length; j++ ) {
        ...
        if ( matcher.apply( matcher, _callArgs ) ) {
          ...
          __matcherContext.become = this.become;
          __matcherContext.behavior = this.behavior;
          __matcherContext.beh = this.beh;
          __matcherContext.create = this.create;
          __matcherContext.raw = this.raw;
          __matcherContext.send = this.send;
          __matcherContext.__this = this;

          var body = '__execute = ' + matcher.__body + ';';
          body += '__execute.apply( __this );';
          
          try {
            vm.runInNewContext( body, __matcherContext );
          } catch ( error ) {
